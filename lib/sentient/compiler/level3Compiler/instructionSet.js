...
var InstructionSet = function (params) {
  ...
  self.select = function () {
    ...
    if (type !== "array") {
      message = "Wrong type for select. Called select on '" + arraySymbol;
      message += "' which is a " + type;
    ...
    if (fn.args.length < 1 || fn.args.length > 3) {
      message = "Tried to call 'select' on '" + arraySymbol + "' with";
      message += " function '" + fn.name + "' that takes " + fn.args.length;
      message += " arguments. 'select' can only be called with functions";
      message += " that take 1, 2 or 3 arguments.";
    ...
    if (fn.returns !== 1) {
      message = "Called 'select' with a function that returns "+ fn.returns;
