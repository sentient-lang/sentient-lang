...
var InstructionSet = function (params) {
  ...
  self.eachPair = function () {
    ...
    if (type !== "array") {
      message = "Wrong type for each_pair. Called each on '" + arraySymbol;
      message += "' which is a " + type;
    ...
    if (fn.args.length !== 2 && fn.args.length !== 4 && fn.args.length !== 6) {
      message = "Tried to call 'each_pair' on '" + arraySymbol + "' with";
      message += " function '" + fn.name + "' that takes " + fn.args.length;
      message += " arguments. 'each_pair' can only be called with functions";
