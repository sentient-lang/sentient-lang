...
var InstructionSet = function (params) {
  ...
  self._call = function (name, width) {
    ...
    for (var i = 0; i < callStack.length; i += 1) {
      functionIds.push(callStack[i][0]);
      functionNames.push(callStack[i][1]);
    }

    if (functionIds.indexOf(fn.id[0]) !== -1) {
      message = "Recursive function calls are not supported.";
      message += " Tried to call \"" + fn.name + "\", but a function with";
      message += " id=" + fn.id[0] + " already exists on the call stack:";

      for (i = callStack.length - 1; i >= 0; i -= 1) {
        var id = callStack[i][0];
        var n = callStack[i][1];
  ...
  var scopedInstructionSet = function (fn) {
    ...
    var newCallStack = callStack.slice(0);
    newCallStack.push(fn.id);
