...
var InstructionSet = function (params) {
  ...
  self.conditionalNils = conditionalNils;
  ...
  self.call = function (instruction) {
    ...
    switch (instruction.type) {
      ...
      case "invariant":
        ...
        break;
      case "define":
        self.define(instruction.name, instruction.args, instruction.dynamic);
        break;
      case "return":
  ...
  self.define = function (name, args, dynamic) {
  ...
  self._call = function (name, width) {
    ...
    if (callStack.indexOf(fn.name) !== -1) {
      var message = "Recursive function calls are not supported.";
      message += " Tried to call \"" + fn.name + "\", but it already exists";
      message += " on the call stack: " + JSON.stringify(callStack);
  ...
  var scopedInstructionSet = function (fn) {
    ...
    var instructionSet = new InstructionSet({
      ...
      functionRegistry: params.functionRegistry,
      codeWriter: params.codeWriter,
      conditionalNils: scopedConditionalNils,
      callStack: callStack.concat(fn.name)
    });
  ...
  var setupInstructionSet = function (instructionSet, fn) {
    ...
    for (var i = 0; i < fn.args.length; i += 1) {
      symbols.push(stack.pop());
    }
  ...
  var copySymbol = function (symbol, newSymbol, from, to) {
    ...
    if (from.conditionalNils[symbol]) {
      to.conditionalNils[newSymbol] = from.conditionalNils[symbol];
    }
  };
