...
var Macros = function (p) {
  ...
  // This needs to be a macro because functions have local scope.
  self.push = function (args, instructions) {
    validateWidth(args, 2, "push");

    var array = args[0];
    var element = args[1];

    var arrayInstructions = p.parse(array);
    var elementInstructions = p.parse(element);

    _.each(arrayInstructions, function (instruction) {
      instructions.push(instruction);
    });

    _.each(elementInstructions, function (instruction) {
      instructions.push(instruction);
    });

    instructions.push({ type: "pushElement" });
  };
