...
var Machine = function (solver) {
  ...
  self.run = function (program, assignments, count) {
    if (typeof count === "undefined") {
      count = 1;
    }

    var constrainedProgram = encodeConstraints(program, assignments);
    var result, decodedResults = [];

    for (var i = 0; i < count || count === 0; i += 1) {
      if (result) {
        constrainedProgram = excludeResult(constrainedProgram, result);
      }

      result = solver.solve(constrainedProgram);
      decodedResults.push(decodeResult(result));
  ...
  var excludeResult = function (program, result) {
    ...
    var clause = _.map(result, function (literal) { return -literal; });
    var dimacsLine = "\n" + clause.join(" ") + " 0";
    var header = "p cnf " + self.numberOfLiterals + " " + self.numberOfClauses;

    return replaceHeader(program, header) + dimacsLine;
  };
...
Machine.run = function (program, assignments, count) {
  return new Machine(MinisatAdapter).run(program, assignments, count);
};
